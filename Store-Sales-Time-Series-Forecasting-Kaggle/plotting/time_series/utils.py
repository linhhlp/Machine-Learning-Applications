"""Plotting functions supporting seasonality and lag/lead features.

    These functions were from Kaggle learntool.
    Some of them were modified to work with this project.
"""
...
# From Lesson 4
def lagplot(
    x, y=None, shift=1, standardize=False, ax=None, atext="", **kwargs
):
...
def plot_lags(
    x, y=None, lags=6, leads=None, nrows=1, lagplot_kwargs={}, **kwargs
):
    import math
...
def make_leads(ts, leads, name="y"):
    return pd.concat(
        {f"{name}_lead_{i}": ts.shift(-i) for i in reversed(range(leads))},
        axis=1,
    )
...
def create_multistep_example(n, steps, lags, lead_time=1):
    ...
    data = data.style.set_properties(
        ["Targets"], **{"background-color": "LavenderBlush"}
